...
export default function DividendScheduleList() {
    ...
    const handleDelete = async (id) => {
        const { error } = await supabase.from('dividend_schedule').delete().eq('id', id)
    ...
    return (
        <div className="space-y-6">
            <DividendScheduleForm initialData={editing} onSave={() => {
                setEditing(null)
                loadSchedules()
            }} />
                    ...
                    <tbody>
                        {schedules.map((row) => (
                            <tr key={row.id} className="border-t">
                                <td className="p-2">{row.asset?.name || 'Unknown'} ({row.asset?.symbol})</td>
                                <td className="p-2">{row.dividend_per_share}</td>
                                <td className="p-2 capitalize">{row.payment_frequency}</td>
                                <td className="p-2">{format(new Date(row.next_payment_date), 'MMM d, yyyy')}</td>
